...
![音声デバイス設定画面](global_audio_hardware_settings.png)

音声の出力先を設定できます。

::: warning
この設定を変更すると再生中のキューがすべて停止されます。
:::

- 出力デバイス
    出力先のデバイスを設定します。
    「既定」を選ぶことでWindowsの設定に従います。

- 出力チャンネル数
    出力デバイスのチャンネル数を設定できます。
    「既定」を推奨します。
    デバイスのサポートしているチャンネル数が表示されます。
    （モノラル設定を行いたい場合は[音声処理](#音声処理)設定から行います。）

- サンプリングレート
    出力デバイスのサンプリングレートを設定します。
    「既定」を推奨します。

- バッファサイズ
    出力デバイスのバッファサイズを設定します。
    「既定」を推奨します。
