...
#[tauri::command]
pub async fn activate_license(app_handle: tauri::AppHandle, license_manager: State<'_, LicenseManager>) -> Result<bool, String> {
    ...
    if let Some(path) = result_rx.await.unwrap() {
        if let Err(e) = license_manager.activate_by_file(path.clone()).map_err(|e| e.to_string()) {
            return Err(e)
